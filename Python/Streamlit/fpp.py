...
# 削除する図形のIDを入力するテキストボックスを表示
if len(st.session_state['draw_data']) != 0:
    delete_shape_id = st.selectbox("選択してください", [value for value in range(1, len(st.session_state['draw_data']) + 1)])
    
    if delete_shape_id:
        delete_check = st.checkbox(label='本当に図形を削除しますか？')
        
    # Deleteボタンがクリックされた場合
    if delete_shape_id and delete_check:
        try:
            delete_shape_id = int(delete_shape_id)
            if delete_shape_id > 0 and delete_shape_id <= len(st.session_state['draw_data']):
                # 削除対象の図形を特定
