...
st.write(st.session_state['draw_data'][0][0]["geometry"])
st.write(st.session_state['draw_data'][0][0]["geometry"]["coordinates"])
# st.write(st.session_state['draw_data'][0][0])
...
if len(st.session_state['draw_data']) != 0:
    ...
    for idx, sdata in enumerate(st.session_state['draw_data']):
        ...
        if len(st.session_state['df']) != 0:
            ...
            folium.GeoJson(sdata,popup=folium.Popup(popup_html)).add_to(tuuka_list[idx])
